...
: "${CR_TOKEN:?Environment variable CH_TOKEN must be set}"
...
main() {
    ...
    readarray -t changed_charts <<< "$(git diff --find-renames --name-only "$latest_tag_rev" -- "$CHART_DIR" | cut -d '/' -f 2 | uniq)"

    if [[ -n "${changed_charts[*]}" ]]; then
        for chart in "${changed_charts[@]}"; do
            echo "Packaging chart '$chart'..."
            package_chart "$CHART_DIR/$chart"
        done
